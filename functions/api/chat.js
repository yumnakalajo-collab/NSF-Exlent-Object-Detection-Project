...
export async function onRequestPost(context) {
  ...
  const { message, history, detections, selectedObject } = body || {};
  ...
  const selectedText = typeof selectedObject === 'string' && selectedObject.trim()
    ? `Selected object for mineral analysis: ${selectedObject.trim()}\n`
    : '';
  if (Array.isArray(detections) && detections.length > 0) {
    const detectionSummary = detections
      .map((d) => `${d.label} (${Math.round((d.value || 0) * 100)}% confidence)`)
      .join(', ');
    userText = `${selectedText}Current camera detections: ${detectionSummary}\n\nUser question: ${message}`;
  } else if (detections !== undefined) {
    userText = `${selectedText}Current camera detections: none right now\n\nUser question: ${message}`;
  }
      ...
      parts: [
        {
          text:
            ...
            'provided. If asked about minerals, batteries, spectral profiles, recycling, or supply chains, ' +
            'be specific, note uncertainty where needed, and avoid inventing exact facts for an unknown brand or model. ' +
            'If no detections are mentioned or relevant, just answer the question normally.',
        },
      ],
    },
  };
