...
export async function onRequestPost(context) {
  ...
  if (!geminiResponse.ok) {
    return jsonResponse({ error: 'Gemini API error (' + geminiResponse.status + '): ' + rawBody.slice(0, 300) }, 502);
  }
  ...
  try {
    ...
    return jsonResponse({ error: 'Gemini returned non-JSON response: ' + rawBody.slice(0, 300) }, 502);
  }

  const candidate = data?.candidates?.[0];
  if (!candidate) {
    const blockReason = data?.promptFeedback?.blockReason;
    return jsonResponse({ error: blockReason ? `Gemini blocked the prompt (${blockReason})` : 'Gemini returned no candidates.' }, 502);
  }

  const reply = extractText(data);

  if (!reply) {
    return jsonResponse({ error: `Gemini returned no usable text (finishReason: ${candidate.finishReason || 'unknown'}).` }, 502);
  }
