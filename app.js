(() => {
  const els = {
    ...
    mineralBody: document.getElementById('mineral-body'),
  };
  ...
  window.__latestMinerals = null; // { device, summary, minerals: [...] } for the top current detection

  let mineralFetchController = null;
  let lastMineralKey = null; // avoid re-fetching/re-rendering for the same label repeatedly
  ...
  function renderResult(result) {
    ...
    if (isDetectionModel) {
      ...
      const sorted = detections.slice().sort((a, b) => b.value - a.value);
      els.readout.innerHTML = sorted
        .map(
          (d) => `
        ...
        .join('');
      handleTopDetectionMinerals(sorted[0].label);
    } else {
      ...
      handleTopDetectionMinerals(top.label);
    }
  }
  ...
  function clearMineralPanel() {
    ...
    if (mineralFetchController) {
      mineralFetchController.abort();
      mineralFetchController = null;
    }
    if (els.mineralPanel) els.mineralPanel.hidden = true;
    if (els.mineralBody) els.mineralBody.innerHTML = '';
  }

  async function handleTopDetectionMinerals(label) {
    if (!els.mineralPanel || !els.mineralBody) return; // markup not present, skip gracefully

    const key = window.normalizeDeviceLabel ? window.normalizeDeviceLabel(label) : label;
    if (!key) {
      clearMineralPanel();
      return;
    }
    if (key === lastMineralKey) return; // already showing this device's data
    lastMineralKey = key;

    // 1. Render instantly from the local fallback dataset, if we have one.
    const localInfo = window.getMineralInfo ? window.getMineralInfo(label) : null;
    if (localInfo) {
      window.__latestMinerals = localInfo;
      renderMineralPanel(localInfo);
    } else {
      renderMineralLoading(label);
    }

    // 2. Then try the server endpoint, which can override with richer/newer data.
    if (mineralFetchController) mineralFetchController.abort();
    mineralFetchController = new AbortController();

    try {
      const res = await fetch(`/api/minerals?device=${encodeURIComponent(key)}`, {
        signal: mineralFetchController.signal,
      });
      if (!res.ok) throw new Error(`Mineral lookup failed (${res.status})`);
      const data = await res.json();
      if (data && Array.isArray(data.minerals) && data.minerals.length) {
        window.__latestMinerals = data;
        renderMineralPanel(data);
      } else if (!localInfo) {
        renderMineralEmpty(label);
      }
    } catch (err) {
      if (err.name === 'AbortError') return;
      // Server data unavailable — local fallback (if any) stays on screen.
      if (!localInfo) renderMineralEmpty(label);
      console.warn('Mineral endpoint unavailable, using local data only:', err);
    }
  }

  function renderMineralLoading(label) {
    els.mineralPanel.hidden = false;
    els.mineralBody.innerHTML = `<div class="mineral-loading">Looking up minerals for "${escapeHtml(label)}"…</div>`;
  }

  function renderMineralEmpty(label) {
    els.mineralPanel.hidden = false;
    els.mineralBody.innerHTML = `<div class="mineral-loading">No mineral data available yet for "${escapeHtml(
      label
    )}". Ask the chat about it!</div>`;
  }

  function renderMineralPanel(info) {
    els.mineralPanel.hidden = false;
    const summary = info.summary ? `<p class="mineral-summary">${escapeHtml(info.summary)}</p>` : '';
    const cards = (info.minerals || [])
      .map(
        (m) => `
        <details class="mineral-card">
          <summary>
          ...
          <div class="mineral-card-body">
            ${m.foundIn && m.foundIn.length ? `<p><strong>Found in:</strong> ${escapeHtml(m.foundIn.join(', '))}</p>` : ''}
            ${m.where ? `<p><strong>Where it comes from:</strong> ${escapeHtml(m.where)}</p>` : ''}
            ${m.purpose ? `<p><strong>What it does:</strong> ${escapeHtml(m.purpose)}</p>` : ''}
    ...
    els.mineralBody.innerHTML = `
      <h3 class="mineral-device-title">${escapeHtml(info.device || 'Device')}</h3>
      ${summary}
      <div class="mineral-card-list">${cards}</div>
    `;
  }
