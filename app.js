(() => {
  ...
  function setLatestDetections(detections) {
    window.__latestDetections = detections;
    window.dispatchEvent(new CustomEvent('detectionsupdated', { detail: { detections } }));
  }
  ...
  async function initClassifier() {
    ...
    classifier.getProjectInfo();
    els.projectName.textContent = 'NSF ExLENT Summer Program Object Detection';
  ...
  function renderResult(result) {
    ...
    if (isDetectionModel) {
      setLatestDetections(detections.map((d) => ({ label: d.label, value: d.value })));
    }
    ...
    if (isDetectionModel) {
      ...
      setLatestDetections(top && top.value >= CONFIDENCE_FLOOR ? [{ label: top.label, value: top.value }] : []);
